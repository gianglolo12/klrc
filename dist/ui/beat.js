...
const PITCH_LO = 5.5;
const PITCH_HI = 11.5;
const bassEnergy = (spectrum, frameIndex) => {
    const frame = spectrum.frames[frameIndex];
    if (!frame)
        return 0;
    let sum = 0;
    for (let b = BASS_BAND_LO; b <= BASS_BAND_HI && b < frame.length; b++)
        sum += frame[b];
    return sum;
};
/**
 ...
 * và hiệu ứng đứng im.
 */
export function beatAt(spectrum, positionMs) {
    if (spectrum.frames.length === 0 || positionMs < 0)
        return 0;
    const index = Math.floor(positionMs / spectrum.frameMs);
    if (index < 0 || index >= spectrum.frames.length)
        return 0;
...
const centroid = (spectrum, frameIndex) => {
    const frame = spectrum.frames[frameIndex];
 ...
 */
export function pitchLevel(spectrum, positionMs) {
    if (spectrum.frames.length === 0 || positionMs < 0)
        return null;
    const index = Math.floor(positionMs / spectrum.frameMs);
    if (index < 0 || index >= spectrum.frames.length)
 ...
 */
export function dominantBand(spectrum, positionMs) {
    if (spectrum.frames.length === 0 || positionMs < 0)
        return null;
    const index = Math.floor(positionMs / spectrum.frameMs);
    ...
    if (average <= 0 || bestValue < average * DOMINANCE_MARGIN)
        return null;
    return best;
}
