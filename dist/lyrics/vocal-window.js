...
export function vocalWindow(spectrum, startMs, endMs) {
    const fallback = { startMs, endMs };
    if (spectrum.frames.length === 0 || endMs <= startMs)
        return fallback;
    ...
    const lastFrame = Math.min(Math.ceil(endMs / spectrum.frameMs) - 1, spectrum.frames.length - 1);
    ...
    for (let i = firstFrame; i <= lastFrame; i++) {
        ...
        let sum = 0;
        for (let b = VOCAL_BAND_LO; b <= VOCAL_BAND_HI && b < frame.length; b++)
            sum += frame[b];
        energy.push(sum);
    }
...
//# sourceMappingURL=vocal-window.js.map