...
import { cleanTitle } from '../lyrics/clean-title.ts'
import { audioPath as cachedAudioPath } from '../lyrics/store.ts'
...
export function friendlyError(err: unknown): Error {
  ...
  for (const [pattern, message] of ERROR_MAP) {
    if (pattern.test(raw)) return new Error(message)
  }
...
export function videoIdFrom(url: string): string {
  try {
    const u = new URL(url)
...
type DumpJson = { title?: string; uploader?: string; channel?: string; duration?: number }
...
export async function resolveYoutube(
  ...
  try {
    ...
    info = JSON.parse(stdout) as DumpJson
  } catch (err) {
