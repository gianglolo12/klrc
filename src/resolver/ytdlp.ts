...
import { cacheDir } from '../lyrics/store.ts'
...
const cachedBinary = (): string => join(cacheDir(), 'bin', 'yt-dlp')

async function runsOk(path: string): Promise<boolean> {
  try {
...
async function inPath(): Promise<string | null> {
  try {
...
async function expectedHash(): Promise<string> {
  ...
  if (!res.ok) throw new Error(`Could not fetch yt-dlp checksums (HTTP ${res.status})`)
  for (const line of (await res.text()).split('\n')) {
    const [hash, name] = line.trim().split(/\s+/)
    if (name === CHECKSUM_NAME && hash) return hash.toLowerCase()
  }
  throw new Error(`Checksum for ${CHECKSUM_NAME} not found in release checksums`)
}

async function download(dest: string, onProgress?: (msg: string) => void): Promise<void> {
...
export async function ytdlpPath(onProgress?: (msg: string) => void): Promise<string> {
  if (resolved) return resolved

  const override = process.env.KLRC_YTDLP
  if (override) {
    if (!(await runsOk(override))) throw new Error(`KLRC_YTDLP is not runnable: ${override}`)
    resolved = override
    return resolved
  }
  ...
  if (!(await runsOk(cached))) throw new Error('Downloaded yt-dlp does not run on this machine.')
  resolved = cached
  return resolved
}

export type YtdlpResult = { stdout: string; stderr: string }
...
export async function ytdlpVersionIfPresent(): Promise<string | null> {
  const candidates = [process.env.KLRC_YTDLP, cachedBinary()].filter(Boolean) as string[]
  const system = await inPath()
  if (system) candidates.unshift(system)
