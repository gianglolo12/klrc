...
import { FFMPEG_HINT, findFfmpeg } from './audio/ffmpeg-path.ts'
import { cacheDir } from './lyrics/store.ts'
import { hasMpv } from './audio/pick-player.ts'
import { ytdlpVersionIfPresent } from './resolver/ytdlp.ts'

const execFileAsync = promisify(execFile)

type Check = { ok: boolean; label: string; detail: string; hint?: string }
...
async function checkMpv(): Promise<Check> {
...
async function checkLrclib(): Promise<Check> {
  try {
    const res = await fetch(
      'https://lrclib.net/api/get?track_name=Shape%20of%20You&artist_name=Ed%20Sheeran&duration=234',
      { headers: { 'User-Agent': 'klrc/0.1.0 (https://github.com/klrc)' }, signal: AbortSignal.timeout(5000) },
    )
...
export async function doctor(): Promise<string> {
  ...
  for (const c of checks) {
    ...
    if (c.hint) lines.push(`     ${' '.repeat(width)}  ${c.hint}`)
  }
