import type { Line, Word } from '../lyrics/types.ts'

/** Chỉ số câu đang hát, hoặc -1 khi nhạc còn ở đoạn dạo đầu. */
export function findActiveLine(lines: Line[], ms: number): number {
  let found = -1
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].startMs <= ms) found = i
    else break
  }
  return found
}
 ...
 */
export function splitSung(words: Word[], ms: number): { sung: string; unsung: string } {
  if (words.length === 0) return { sung: '', unsung: '' }
