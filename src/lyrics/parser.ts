import type { Line, Lyrics, Word } from './types.ts'
...
const fracToMs = (frac: string | undefined): number =>
  frac ? Number(frac.padEnd(3, '0').slice(0, 3)) : 0

const toMs = (min: string, sec: string, frac?: string): number =>
  Number(min) * 60_000 + Number(sec) * 1000 + fracToMs(frac)

/** Tách timing từng chữ của enhanced LRC. Trả mảng rỗng nếu dòng không có. */
function parseWords(body: string, lineEndMs: number): Word[] {
  const found: { startMs: number; text: string }[] = []
  for (const m of body.matchAll(WORD_STAMP)) {
    const text = m[4].trim()
    if (text) found.push({ startMs: toMs(m[1], m[2], m[3]), text })
  }
...
export function parseLrc(text: string): Lyrics {
  const raw: { startMs: number; body: string }[] = []
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    for (const s of stamps) raw.push({ startMs: toMs(s[1], s[2], s[3]), body })
  }
