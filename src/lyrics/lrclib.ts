...
export type LyricsQuery = { title: string; artist: string; durationMs: number }
...
function fromRecord(rec: LrclibRecord): LyricsResult {
  if (rec.instrumental) return { kind: 'none' }
  if (rec.syncedLyrics) return { kind: 'synced', lrc: rec.syncedLyrics }
  if (rec.plainLyrics) return { kind: 'plain', text: rec.plainLyrics }
  return { kind: 'none' }
}
...
export async function fetchLyrics(
  ...
  try {
    ...
    const exact = (await getJson(`${BASE}/api/get?${params}`, fetchImpl)) as LrclibRecord | null
    if (exact) {
    ...
    const list = (await getJson(`${BASE}/api/search?${search}`, fetchImpl)) as
      | LrclibRecord[]
      | null
    if (!Array.isArray(list)) return { kind: 'none' }
