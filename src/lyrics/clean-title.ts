...
const JUNK =
  /^(4k|8k|hd|hq|official|official\s+(video|mv|m\/v|audio|lyric\s*video|visualizer)|mv|m\/v|lyrics?|lyric\s*video|audio|video|visualizer|full|full\s+album|remaster(ed)?(\s*\d{2,4})?|explicit)$/i

/** Phần trước dấu gạch dài hơn mức này thì không phải tên nghệ sĩ. */
const MAX_ARTIST_LEN = 40

const isJunk = (s: string) => JUNK.test(s.trim())

export function cleanTitle(raw: string): { title: string; artist?: string } {
  // Phần sau dấu | luôn là mô tả kênh hoặc quảng cáo.
  let s = raw.split('|')[0]

  // Bỏ (...) và [...] nếu bên trong chỉ là mô tả bản phát hành.
  s = s.replace(/[([]([^)\]]*)[)\]]/g, (all, inner: string) => (isJunk(inner) ? '' : all))
